...
StatusCode LCD::send_string(double value, int decimals)
{
    string text = string_formatter::to_string(value, decimals);

    return send_string(text);

} // end of "send_string(double, int = 3)"


StatusCode LCD::send_string(int value)
{
    string text = string_formatter::to_string(value);
